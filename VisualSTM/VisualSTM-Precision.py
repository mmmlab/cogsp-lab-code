...
myDlg = gui.Dlg(title="Visual Short Term Memory (Precision)",size=(1, 1))
...
def write_file(filename):
    ...
    ws.cell(row=1,column=1).value = "ExperimentName: Visual Short Term Memory (Precision)"
    ws.cell(row=2,column=1).value = "SubjectID:%s"%idn
    ws.cell(row=3,column=1).value = "SessionNumber:%s"%sessnumber
    ws.cell(row=4,column=1).value = "Inter-display time:%s"%pause
    # write column headers
    headers = ['Trial','SetSize','Response','DeltaTilt']
    ...
    for r in range(0,len(response)):
        ...
        ws.cell(irow, 2).value = nbarstostore[r]
        ws.cell(irow, 3).value = response[r]
        ws.cell(irow, 4).value = tiltchange[r]
