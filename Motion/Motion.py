...
def write_file(filename,occ_contrast,square_contrast):
    ...
    headers = ['Trial','SquareContrast','OccluderContrast','Response']
...
def show_instruction():
    ...
    Each display will contain four moving line segments.
    
    The term "coherent" means that the four lines appear to be part
    of the same shape and appear to be moving as a rigid object.
    
    The term "incoherent" means that the four lines appear to be 
    separate objects and moving indepedently.
    
    After viewing each trial, press either the 1, 2, or 3 key to indicate
    the appearance of the motion of the four lines, where 1 means 
    "incoherent", 2 means somewhere in between, and 3 means "coherent".
    
    PRESS ANY KEY TO START...
    """

    block_instructions = visual.TextStim(win,text=block_instruction_text,
        color='White',pos=(0,0),alignText='center', wrapWidth = 100)
...
if myDlg.OK:
    ...
    square_contrast = float(square_contrast)
    occ_contrast = float(occ_contrast)
    file=''
    file+=name
